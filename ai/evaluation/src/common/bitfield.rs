use std::cmp::{max, min};
use std::ops::{BitAnd, BitAndAssign, BitOr, BitOrAssign, BitXor, Fn, Index, IndexMut, Not};
...
impl BitField {
    ...
    pub fn move_right(&mut self, i: u64) -> &mut Self {
        self.field.iter_mut().for_each(|x| *x >>= i);
        self
    }

    pub fn move_left(&mut self, i: u64) -> &mut Self {
        self.field.iter_mut().for_each(|x| *x <<= i);
        self
    }
