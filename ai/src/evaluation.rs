...
#[allow(dead_code)]
pub fn passable<'a, I: Iterator<Item = &'a Vec2>>(
    (begin, end): (Vec2, Vec2),
    objects: I,
) -> Option<f32> {
...
mod tests {
    ...
    fn test_passable() {
        ...
        assert_eq!(
            passable((begin, end), [vec2(2.0, 0.0)].iter()),
            Some(sqrt(2.0))
        );
        assert_eq!(
            passable((begin, end), [vec2(0.0, 2.0)].iter()),
            Some(sqrt(2.0))
        );
        assert_eq!(passable((begin, end), [vec2(0.0, 0.0)].iter()), None);
        assert_eq!(
            passable((end, begin), [vec2(2.0, 0.0)].iter()),
            Some(sqrt(2.0))
        );
        assert_eq!(
            passable((end, begin), [vec2(0.0, 2.0)].iter()),
            Some(sqrt(2.0))
        );
